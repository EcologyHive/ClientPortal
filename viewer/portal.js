...
function SiteMap({ site, surveyId, layers }) {
  ...
  // Offline handover degradation (architecture plan §3): basemap tiles aren't bundled into an
  // offline export, so when they're unreachable the map still shows survey geometry against a
  // neutral background rather than Leaflet's blank/black void, with an explicit banner rather than
  // a silently broken map. navigator.onLine catches the fully-offline case; the tileerror listener
  // also catches online-but-blocked (e.g. a firewalled handover machine).
  ...
  useEffect(() => {
    ...
    const tileLayer = SR.Map.makeTileLayer(SR.Map.DEFAULT_BASE_LAYER);
    tileLayer.on('tileerror', () => setTilesUnavailable(true));
    tileLayer.on('tileload', () => setTilesUnavailable(false));
    tileLayer.addTo(map);
    mapRef.current = map;
  ...
  return h('div', { className: 'map-picker', style: { height: 400, position: 'relative' } },
...
function LoadSiteScreen({ onLoaded }) {
  ...
  async function onFile(e) {
    ...
    try {
      ...
      // No migration here - deliberately not in the IP-bounded viewer build (see the architecture
      // plan's §5). SurveyReview's own export functions always hand over already-current-shape
      // data, so this only ever matters for a genuinely old .json exported by a since-superseded
      // SurveyReview version - such a file would show its existing fields as-is rather than being
      // backfilled with defaults for anything added since.
...
function App() {
  ...
  // output with preloaded project." No migration on this path either, same reasoning as
  // LoadSiteScreen's own comment - SurveyReview's own live state is always already current-shape by
  // the time it exports.
  const [site, setSite] = useState(() => window.__PRELOADED_SITE__ || null);
