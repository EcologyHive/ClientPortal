// Deliberately trimmed, viewer-safe mirror of SurveyReview's own dsp.js - see the architecture plan
// and data-model.js's own header comment for the reasoning. Mirrors SurveyReview/dsp.js as of
// commit 701ca41 (2026-09-08): computeSpectrogram, renderSpectrogramImageData, frameIndexForTime,
// binIndexForFreq, DEFAULT_FLOOR_DB - exactly what SonogramPopup calls (verified by grep, not
// guesswork). The analyst's hand-measurement and call-shape-classification tooling
// (measureBox/classifyShape/refineDurationFromOscillogram/heterodyneMix), the oscillogram waveform
// helper, and the off-main-thread worker path are deliberately excluded - none of it is used by
// this read-only viewer, and classifyShape in particular is exactly the kind of "data-processing/
// classification algorithm" the IP boundary asked to keep internal.
window.SR = window.SR || {};
